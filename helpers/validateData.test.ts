...
import { Post, PostsData } from '../hooks/usePosts';
...
describe('Validate Data helper', () => {
  ...
  describe('when the data is empty', () => {
    it('should be false', () => {
      expect(IsValidData({} as PostsData)).toBe(false);
    });
  });
  describe('when the post are empty', () => {
    it('should be false', () => {
      expect(IsValidData({ posts: [] })).toBe(false);
    });
  });
  describe('when there are posts', () => {
    it('should be true', () => {
      expect(IsValidData({ posts: [post] })).toBe(true);
    });
  });
});
